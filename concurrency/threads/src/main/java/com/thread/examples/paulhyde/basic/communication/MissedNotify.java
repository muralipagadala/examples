...
/**
 * This example shows how to avoid thread waiting due to the missed notification.
 * I guess it may not solve the missed notification.
 * 
 * Following is from book.
 * 
 * In this example, okToProceed is set true by threadB long before threadA enters waitToProceed(). 
 * When threadA evaluates the while expression, 
 * it determines that no waiting is necessary and skips the body of the loop. 
 * The notification is still missed, but the indicator variable okToProceed prevents threadA from waiting.
 *
 */
